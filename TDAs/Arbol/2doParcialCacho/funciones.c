...
{
    return ((tIndice*)v1)->clave - *((long*)v2); //estoy errandole acá
}
               ...
               int (*cmp)(const void *,const void*))
{
    ...
    fpEF = fopen(archivoEstadosFinan,"r+b"); //es el cuentas.dat que en realidad lo nombré de otra manera xd
    ...
    {
        fread(&prod,sizeof(tProducto),1,fpEF); //leo esa posición
        buscarIndArbolBinBusq(p,&prod,&ind,sizeof(tIndice),cmp);
        fseek(fpEF,(ind.pos * ((int)sizeof(tProducto))),SEEK_CUR);///está bien porque va a mover la pos del indice dentro de ese archivo

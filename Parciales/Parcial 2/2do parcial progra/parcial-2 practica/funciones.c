...
int compInd(const void *v1, const void *v2)
{
    const tIndice* ind1 = (tIndice*)v1;
    const tIndice* ind2 = (tIndice*)v2;
                          ...
                          int (*comp)(const void *, const void *))
{
    ...
    if(res > 0) //tengo un 3 y un 7 - si es mayor el dato del arbol, lo tengo que buscar por la izq porque el dato es más chiquito
    {
        return recuperarDeArbol(&(*p)->izq, bus, tamBus, comp);
    }
    else if(res < 0) //- si es menor el dato del arbol, lo tengo que buscar por derecha
    {
